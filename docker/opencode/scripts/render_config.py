...
def _flight_auth_header_value(header_name: str) -> str:
    """Return the opencode config value for the MCP auth header (业务 SKILL 自定义)."""
    if header_name.lower() == "authorization":
        prefix = os.environ.get("MCP_TOKEN_AUTH_PREFIX", "Bearer").strip()
        return f"{prefix} {{env:MCP_TOKEN}}" if prefix else "{env:MCP_TOKEN}"
    return "{env:MCP_TOKEN}"


def _flight_mcp_server_from_env() -> dict | None:
    """Build the default MCP server. 业务 SKILL 通过 env 自定义 endpoint/header.

    Always register the server. Auth is still read through ``{env:MCP_TOKEN}``,
    but the model must be able to see the native MCP tools before the first
    per-request token write/reload succeeds.
    """
    header_name = os.environ.get("MCP_TOKEN_HEADER", "token").strip() or "token"
    endpoint = os.environ.get("MCP_TOKEN_ENDPOINT", "").strip()
    if not endpoint:
        return None
    return {
        "type": "remote",
        ...
        "oauth": False,
        "enabled": True,
        "timeout": int(os.environ.get("MCP_TOKEN_TIMEOUT_MS", "30000")),
    }
...
def _mcporter_bridge_server(policy: dict) -> dict:
    ...
    upstream_servers = cfg.get("servers") or os.environ.get("MCPORTER_SERVERS")
    if not upstream_servers:
        # 没配 server, mcporter bridge 不可用
        return {"type": "local", "command": ["true"], "enabled": False}
...
def _build_mcp_servers(policy: dict) -> dict:
    ...
    default = None if _mcporter_enabled(policy) else _flight_mcp_server_from_env()
    if default:
        # 通用 MCP server (业务 SKILL 通过 MCP_TOKEN_ENDPOINT 声明).
        servers.setdefault("default_mcp", default)
    return servers
...
def _build_tool_output(policy: dict, *, has_mcp_servers: bool) -> dict | None:
    ...
    业务 MCP 工具可能返回大段 JSON. 如果 opencode 截断, 基座 CardRenderer
    可能拿到不完整数据. 配置由 env 声明, 不写死.
    """
...
def render(policy: dict) -> dict:
    ...
    # 业务 MCP tools directly instead of writing curl.
