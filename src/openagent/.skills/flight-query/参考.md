...
> 版本：v0.2  状态：**MCP 接入中（query_flight_basic 已落地）**  最后更新：2026-06-02
> 数据来源：`AI订票-01 机票流程图.png`、`fh-travel-ai-FUNCTION-MODULES.md`、`fh-travel/AI订票流程-功能接口映射表.md`、`references/query-flight-api.md`、`references/query-flight-response.json`、`references/query_flight_basic.schema.json`
...
## 4. MCP 工具集（已接入：query_flight_basic；其余待接入）

> **接入进度**：§4.2 已落地 `domestic-booking-mcp.query_flight_basic`（基于 `references/query-flight-api.md` 入参 + `references/query-flight-response.json` 响应）。
> 完整 JSON Schema 见 `references/query_flight_basic.schema.json`（含 `input_schema` / `output_schema` / `examples`）。
> 其余 16 个工具保持占位，等用户提供定义后逐个展开为：
> - 工具名称 / 描述
> - 输入 JSON Schema
> - 输出 JSON Schema
> - 错误码
...
### 4.2 `domestic-booking-mcp.query_flight_basic`（已落地）

> **来源**：`references/query-flight-api.md`（入参）+ `references/query-flight-response.json`（出参，282 个舱位 / 122 个航段样本）+ `references/query_flight_basic.schema.json`（完整 JSON Schema）。
> 底层调用 `pub.TmsRequestMananger.shopping`（POST `/api/air/domestic/shopping`），Token 由 MCP Gateway 注入，模型不需要关心。

#### 4.2.1 工具元信息

| 字段 | 值 |
|---|---|
| 名称 | `domestic-booking-mcp.query_flight_basic` |
| 描述 | 根据出发地 / 目的地 / 日期，查询国内航班及舱位列表。单程/往返均可。 |
| 入口守卫（allowed_states） | `OD_CONFIRMED`、`FLIGHT_LISTED`、`FLIGHT_SELECTED`（重查/换 OD 时） |
| 写 ctx 副作用 | `ctx.resultSetId` = `data.serialNumber`；`ctx.flightList` = `data.flightList` 精简版 |
| 幂等 | 纯查询，无写库副作用。`idempotencyKey` 仍建议传（用于日志去重） |

#### 4.2.2 输入 JSON Schema（精简，模型只看这些字段）

```jsonc
{
  "type": "object",
  "required": ["fromCity", "toCity", "flyDate"],
  "properties": {
    "fromCity":      { "type": "string",  "description": "出发城市 IATA 三字码，例如 'SZX'" },
    "toCity":        { "type": "string",  "description": "到达城市 IATA 三字码，例如 'BJS1'" },
    "flyDate":       { "type": "string",  "format": "date", "description": "出发日期 yyyy-MM-dd" },
    "returnDate":    { "type": "string",  "format": "date", "description": "【仅往返】返程日期" },
    "cabClass":      { "type": "string",  "enum": ["", "Y", "C", "F", "W"],
                        "description": "舱等等级：Y=经济 / C=公务 / F=头等 / W=高端经济；空=不限" },
    "journeyType":   { "type": "integer", "enum": [0, 1],
                        "description": "0=单程（默认），1=往返" }
  }
}
```

> 入参里 `dataSource` / `cacheType` / `official` / `showPriceText` / `publicWeb` / `returnFcy` / `clientIdSpecify` / `dataSourceShowAllCab` / `choiceHf` 等是 **系统标志位**，由 MCP Gateway 默认注入，模型不感知。

#### 4.2.3 输出 JSON Schema（精简，模型只看这些字段）
...
```jsonc
{
  ...
  "properties": {
    "errorCode":  { "type": "string", "description": "'0'=成功；其他=业务错误码" },
    "errorMsg":   { "type": "string", "description": "errorCode≠0 时向用户展示" },
    "data": {
      "type": "object",
      "required": ["serialNumber", "flightList"],
      "properties": {
        "serialNumber":   { "type": "string", "description": "查询流水号 → 写入 ctx.resultSetId" },
        "flightList": {
          "type": "array",
          "description": "航班分组（按起飞时间升序）",
          "items": {
            "type": "object",
            "required": ["flightId", "flight"],
            "properties": {
              "flightId": { "type": "string", "description": "航班号，向用户展示用" },
              "flight":   { "type": "array", "description": "航段列表（单程=1段；往返=2段）",
                            "items": {
                "type": "object",
                "required": ["flightId", "airLine", "flyDate", "arrDate", "fromPort", "type", "cabins"],
                "properties": {
                  "flightId":  { "type": "string",  "description": "航段航班号" },
                  "airLine":   { "type": "string",  "description": "起降机场组合码 'FROMTOT'，如 'SZXPKX'" },
                  "flyDate":   { "type": "string",  "format": "date-time", "description": "起飞 'yyyy-MM-dd HH:mm:ss'" },
                  "arrDate":   { "type": "string",  "format": "date-time", "description": "到达 'yyyy-MM-dd HH:mm:ss'" },
                  "fromPort":  { "type": "string",  "description": "出发航站楼，如 'T3'" },
                  "toPort":    { "type": "string",  "description": "到达航站楼，如 'T2'" },
                  "type":      { "type": "string",  "description": "机型代码，如 '325'（可查 types 字典）" },
                  "shareId":   { "type": "string",  "description": "共享航班主航班号；空=非共享" },
                  "stop":      { "type": "integer", "description": "经停次数；0=直飞" },
                  "mile":      { "type": "integer", "description": "航程公里数（一般不展示）" },
                  "fullPrice": { "type": "number",  "description": "Y 舱全价票面（差标用）" },
                  "mealCode":  { "type": "string",  "description": "B/L/D/S/M 餐食代码" },
                  "cabins": {
                    "type": "array",
                    "description": "该航段可选舱位（按价格升序）",
                    "items": {
                      "type": "object",
                      "required": ["cabId", "cab", "cabClass", "cabName", "num", "price"],
                      "properties": {
                        "cabId":          { "type": "string",  "description": "舱位报价 ID，choose_cabin 回传此值" },
                        "cab":            { "type": "string",  "description": "IATA 订座类：Y/C/F/S/G…" },
                        "cabClass":       { "type": "string",  "enum": ["F", "C", "C1", "W", "W1", "Y", "Y1"],
                                            "description": "舱等大类；差标判断用此字段" },
                        "cabName":        { "type": "string",  "description": "舱位中文名（向用户展示）" },
                        "num":            { "type": "string",  "description": "余票：'A'=≥9，'B'=8，数字<9时直显" },
                        "price":          { "type": "number",  "description": "实际售价（含税）" },
                        "normalPrice":    { "type": "number",  "description": "票面价" },
                        "clientService":  { "type": "number",  "description": "机建+燃油（已含在 price）" },
                        "priceType":      { "type": "integer", "enum": [0, 1], "description": "0=普通；1=特价（退改严格）" },
                        "weight":         { "type": "string",  "description": "免费托运 kg" },
                        "portableWeight": { "type": "string",  "description": "免费手提 kg" },
                        "luggage":        { "type": "string",  "description": "行李原始说明" },
                        "refund":         { "type": "string",  "description": "退票规则摘要" },
                        "change":         { "type": "string",  "description": "改期规则摘要" },
                        "refundRules":    { "type": "string",  "description": "结构化退票规则（部分舱位）" },
                        "changeRules":    { "type": "string",  "description": "结构化改期规则（部分舱位）" },
                        "otherProductPolicyIds": { "type": "string", "description": "增值产品策略 ID（逗号分隔）" }
                      }
                    }
                  }
                }
        ...
        "citys":    { "type": "array", "description": "城市/机场字典（反查 cityCode→cityName/airPortName）" },
        "airways":  { "type": "array", "description": "航司字典（反查 companyNo→companyName）" },
        "types":    { "type": "array", "description": "机型字典（反查 type→name）" }
      }
    }
  }
}
...
> **已剔除的噪音字段**（模型不读，由 pub 层透传给前端或审计）：`baf/acf`、`fareBasis`、`productName/productLabel`、`adultLimit/ageLimit/passengerCertLimit`、`unitPrice/upgradePrice/dataMask`、`enterpriseId/serviceId/issue`、`otherProductStr/otherProduct`、`pickUpRemark/vipLoungeRemark/wifiRemark/flightDelayInsuranceRemark/flightAccidentInsuranceRemark`、`noMeal`、`flightinfoid/priceinfoid/prdSubType`、`goMore`、`priceKind`、`clientID/dataCache/ifDelSdNoCabs/thirdPricePoundage/frCityToCityAirLineList/priceTextJs/policyDocumentList/cityWeatherList/policyDescList/fcyList/businessAirCabList/bigCustomerRemindType`。
> 完整原始 schema 在 `references/query_flight_basic.schema.json`，需要时回查。

#### 4.2.4 错误码

| 错误码 | 含义 | 重试 | 兜底动作 |
|---|---|---|---|
| `errorCode≠0` + `tmsErrorCode=""` | 业务级失败（无航班 / 参数非法 / 协议关闭） | ❌ | 按 `errorMsg` 提示用户；F2 CANNOT_ORDER |
| `TMS_TIMEOUT` | 上游超时 | ✅ ×3，指数退避 | 仍失败 → F2 + "系统繁忙" |
| `PRICE_UNAVAILABLE` | 协议价未下发 | ❌ | 提示用户换日期或换舱等 |
| `OD_NOT_SERVED` | 该 OD 不在产品范围内 | ❌ | F2 + 提示"该航线暂不支持" |

#### 4.2.5 与 §2.1 / §3.3 对应

| 触发该工具的状态 | 后续动作 |
|---|---|
| `OD_CONFIRMED` | 直接调用 → 写入 `ctx.flightList` + `ctx.resultSetId` → 推 `FLIGHT_LIST` 卡片 → 状态进 `FLIGHT_LISTED` |
| `FLIGHT_LISTED` | 用户要求"换时间/换舱等"重查 → 覆盖 `ctx.flightList`（保留 `resultSetId`） |
| `FLIGHT_SELECTED` | 用户临时改 OD/日期 → 重查后回到 `FLIGHT_LISTED`（需清空 `segments`） |
        ...
        "flight": [
          {
            "flightId": "MU6662",
            "airLine": "SZXPKX",
            "flyDate": "2026-06-04 06:55:00",
            "arrDate": "2026-06-04 10:10:00",
            "fromPort": "T3",
            "type": "325",
            "stop": 0,
            "shareId": "",
...
#### 4.2.7 模型阅读指引（Prompt Hint）

> 读取 `data.flightList[*].flight[*]` 时，**优先**：`flightId` `flyDate` `arrDate` `fromPort/toPort` `stop` `cabins[].cabId/cabName/cabClass/num/price`。
> 不读：`baf/acf/mile/fullPrice/mealCode/weight/portableWeight` 等附属字段（除非用户明确问"行李/餐食/经停"）。
> 字典（`citys/airways/types`）只在用户问"这是哪个航司/哪个机场/什么机型"时按 key 反查。
...
- [x] §2.1 状态表里所有 13 个状态都有明确入口守卫
- [ ] §4.1 每个 MCP 工具都有完整 `input_schema` / `output_schema` / `errors`
  - [x] `query_flight_basic`（§4.2）
  - [ ] 其余 16 个工具
...
| v0.2 | 2026-06-02 | **接入 `query_flight_basic`**：基于 `references/query-flight-api.md` + `references/query-flight-response.json` 提炼出 LLM 友好的 JSON Schema（剔除 30+ 噪音字段，保留 flightId/cabId/cabClass/price/time 等核心），落地到 §4.2 + `references/query_flight_basic.schema.json` |

> **下一步**：用户提供剩余 16 个 MCP 工具定义后，按同样模板（精简 schema + 错误码 + 状态守卫）逐个补齐 §4.2。
