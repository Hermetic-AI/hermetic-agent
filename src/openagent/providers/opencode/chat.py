...
#
# P0 重构: 改为基于 OpenCodeAdapter 实例的 set, 避免跨 agent 池污染
# (旧全局变量在测试/多 Agent 场景会互相干扰). 兼容旧 import path
# `from openagent.providers.opencode.chat import _FLIGHT_CARD_EMITTED`
# 仍可用 — 它现在是 lazy dict[adapter_id, set[session_id]] 包装, 对外
# 保持 set-like 接口.
import weakref

_FLIGHT_CARD_ADAPTERS: weakref.WeakKeyDictionary[object, set[str]] = (
    weakref.WeakKeyDictionary()
)


class _FlightCardEmittedView:
    """``_FLIGHT_CARD_EMITTED`` 的 view 对象 — 兼容旧 ``set[str]`` API.

    内部按 adapter 实例隔离, 但暴露 ``add`` / ``__contains__`` 等 set 接口,
    跟历史 tests (``test_opencode_chat_flight_card.py``) 的 ``set`` 假设
    兼容. 实际写入前需调用 ``bind(adapter)`` 关联到具体 adapter.
    """

    def __init__(self) -> None:
        self._bound: set[str] | None = None

    def bind(self, adapter: object) -> set[str]:
        bucket = _FLIGHT_CARD_ADAPTERS.get(adapter)
        if bucket is None:
            bucket = set()
            _FLIGHT_CARD_ADAPTERS[adapter] = bucket
        self._bound = bucket
        return bucket

    def _bucket(self) -> set[str]:
        if self._bound is None:
            # 没 bind, 退化到进程级 set (旧行为)
    ...
    def add(self, session_id: str) -> None:
        self._bucket().add(session_id)

    def discard(self, session_id: str) -> None:
        self._bucket().discard(session_id)

    def __contains__(self, session_id: str) -> bool:
        return session_id in self._bucket()
    ...
    def clear(self) -> None:
        """清空当前 bucket (兼容 set.clear() API).

        测试场景用得到 (跨 test 清理状态). 实际生产**不建议**清
        (会丢失其他 adapter 的 dedup 信息) — 推荐用 ``bind(adapter)``
        给每个 adapter 一个独立 bucket, 互不干扰.
        """
        self._bucket().clear()


# 兼容旧 tests: ``_FLIGHT_CARD_EMITTED`` 仍可作为 set-like 对象使用
_FLIGHT_CARD_EMITTED = _FlightCardEmittedView()
...
async def stream_chat(
    ...
    *,
    ...
    # P0 重构: 把 _FLIGHT_CARD_EMITTED 绑到当前 adapter 实例,
    # 避免跨 adapter (测试多实例 / 多 agent 池) 互相污染.
    _FLIGHT_CARD_EMITTED.bind(adapter)
