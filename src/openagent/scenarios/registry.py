...
class ScenarioRegistry:
    ...
    def load_from_db(self, rows: list[dict[str, Any]] | None = None) -> list[ScenarioConfig]:
        """从 DB 加载并覆盖 YAML 默认.

        当前为 stub — P7 阶段会接 Postgres storage. 入参 rows 暂不消费,
        保持向后兼容.
        """
        logger.info("load_from_db_stub", rows=len(rows or []))
        return []
