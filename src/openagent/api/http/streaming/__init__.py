...
P1 重构: 随 api/ 拆 4 子包, 本模块从 ``openagent.api.streaming`` 移到
``openagent.api.http.streaming``. 新代码直接 ``from openagent.api.http.streaming import ...``;
旧 ``openagent.api.streaming`` 仍通过 ``api/http/routes.py`` (原 shim) 兼容
(其实它现在指 ``openagent.api.http.routes``, 不再是 streaming shim — 见
``api/http/routes.py`` 顶部 docstring).
"""
from openagent.api.http.streaming.ask_user import (
    ASK_USER_TOOL_NAMES,
    is_ask_user_tool,
    stream_with_ask_user_intercept,
)
from openagent.api.http.streaming.done_gate import DoneGate
from openagent.api.http.streaming.keepalive import (
    DEFAULT_KEEPALIVE_INTERVAL,
    KEEPALIVE_SSE_LINE,
    stream_with_keepalive,
)
from openagent.api.http.streaming.route_hints import (
