...
def has_complete_route_hint(message: str) -> bool:
    ...
    if not message:
        return False
    return bool(_FH_ROUTE_RE.search(message)) and _parse_date(message)


def should_bypass_hitl_placeholder(scenario: Any, message: str) -> bool:
    """判定: 当前 scenario + 用户消息 是否可绕过 HITL 占位卡片.

    仅针对 ``fh_domestic_flight_booking`` scenario; 其他 scenario 永远不
    绕 (没有 OD_INPUT 占位卡片).
    """
