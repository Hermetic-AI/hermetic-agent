...
def collect_readiness(request: Request) -> dict:
    ...
    if getattr(request.app.ctx, "scenarios_initialized", False):
        scenario_registry = getattr(request.app.ctx, "scenario_registry", None)
        n_scenarios = len(scenario_registry.list_all()) if scenario_registry else 0
        if n_scenarios:
            checks.append(_check_component("scenarios", True, f"{n_scenarios} scenario(s) loaded"))
        else:
            checks.append(_check_component(
                "scenarios", False,
        ...
        checks.append(_check_component(
            "scenarios", False,
            f"scenario subsystem not initialized: {err}",
        ))
