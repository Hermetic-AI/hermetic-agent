...
class SkillRegistry:
    ...
    def _parse_skill_md(self, path: Path) -> Skill | None:
        ...
        from hermetic_agent.skills.frontmatter import parse_skill_md
    ...
    def load_from_db(
        ...
        *,
        ...
        """从 DB 加载 Skill, 若 DB 无数据则降级到文件系统.

        Args:
            skill_service: SkillService 实例 (store/services/skill_service.py).
            fallback_paths: DB 无数据时降级加载的文件系统路径.
        ...
        if skill_service is not None:
            import asyncio
            try:
                db_skills = asyncio.get_event_loop().run_until_complete(
                    skill_service.list_active()
                )
            except RuntimeError:
            ...
            for s in db_skills:
                skill = Skill(
                    ...
                    mcp_tools=list(s.mcp_tools) if s.mcp_tools else [],
                    source=s.source,
                )
                self._skills[skill.name] = skill
                db_loaded += 1
        if db_loaded > 0:
            logger.info("skills_loaded_from_db", count=db_loaded)
            return list(self._skills.values())
        if fallback_paths:
            logger.info("skills_fallback_to_filesystem", paths=fallback_paths)
            return self.load_from_paths(*fallback_paths)
        return []
