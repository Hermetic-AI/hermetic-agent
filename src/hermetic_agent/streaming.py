"""hermetic_agent.streaming — 兼容 shim (历史路径).

P0 架构调整: ``streaming.py`` 物理合并到 ``hermetic_agent.providers.streaming``.
新代码请用 ``from hermetic_agent.providers.streaming import ...`` 直接 import;
老 import 路径 ``from hermetic_agent.providers.streaming import ...`` 仍可用, 但会
...
注意: 不能直接 ``from hermetic_agent.providers.streaming import ...`` — 因为
``hermetic_agent/providers/__init__.py`` 会再 import agent_bridge → claude_code_*,
那些模块**也** import 本 shim, 触发循环. 这里用 ``importlib.util`` 直接
按文件路径加载, 完全绕开 ``hermetic_agent.providers`` 包的 ``__init__``.
"""
...
_warnings.warn(
    "Importing from 'hermetic_agent.streaming' is deprecated. "
    "Use 'hermetic_agent.providers.streaming' instead. This shim will be removed in a future release.",
    DeprecationWarning,
    stacklevel=2,
)
...
_spec = _importlib_util.spec_from_file_location("hermetic_agent.providers.streaming", _STREAMING_FILE)
_mod = _importlib_util.module_from_spec(_spec)
# 先把模块注册到 sys.modules, 避免内部 import 再次走 spec_from_file_location
import sys as _sys

_sys.modules["hermetic_agent.providers.streaming"] = _mod
