...
def _check_skills(cfg: ScenarioConfig) -> list[str]:
    ...
    ``src/hermetic_agent/.skills/`` 副本 (Dockerfile build 时 COPY 进去).
    路径不同 (sandbox 那边是 work/shared, Hub 这边是 src/hermetic_agent/.skills),
    在 Hub 端用 ``${WORK_SHARED}/skills`` 校验会永远 miss.
    实际运行由 SkillRegistry (从 src/hermetic_agent/.skills/ 加载) 处理.
    """
    ...
    if not root:
        logger.warning(
            ...
            note="resource_dirs.skills is empty but execution.skills is non-empty; "
                 "Hub will use bundled src/hermetic_agent/.skills/ instead",
        )
        return []
    for sk in cfg.execution.skills:
        ...
        if not skill_md.exists():
            logger.warning(
                ...
                skill=sk,
                path=str(skill_md),
                note="Hub 端 skill 源在 src/hermetic_agent/.skills/, sandbox 端在 ${WORK_SHARED}/skills",
            )
    return []
