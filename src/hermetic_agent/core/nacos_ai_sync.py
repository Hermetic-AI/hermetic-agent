...
class NacosAISync:
    ...
    async def _sync_mcp_servers(self, mcp_registry: Any) -> bool:
        ...
        try:
            ...
            service_ref = McpServiceRef(
                ...
                serviceName="hermetic-agent",
            )
            remote_config = McpServerRemoteServiceConfig(
                serviceRef=service_ref,
            )
            server_spec = McpServerBasicInfo(
                name="hermetic_agent-mcp",
                description="hermetic-agent MCP tools",
                protocol="mcp",
                version="1.0.0",
    ...
    async def _sync_agents(self, agent_bridge: Any) -> bool:
        ...
        try:
            ...
            for name, agent_cfg in agents.items():
                ...
                card = AgentCard(
                    name=name,
                    description=f"hermetic-agent sandbox: {name}",
                    url=base_url,
                    version="1.0.0",
    ...
    async def _sync_prompts(self) -> bool:
        ...
        try:
            ...
            param = GetPromptParam(prompt_key="hermetic_agent-system")
