...
``hermetic_agent.api.http.streaming`` 子包, controller 自身只保留 endpoint
...
from hermetic_agent.api.http.schemas import (
    ChatRequest,
    ChatResponse,
    ErrorResponse,
    get_bridge,
)
from hermetic_agent.api.http.streaming import (
    ASK_USER_TOOL_NAMES,
    DoneGate,
    is_ask_user_tool,
...
from hermetic_agent.store.dto.chat_turn import CreateChatTurnRequest
from hermetic_agent.store.dto.message import CreateMessageRequest
from hermetic_agent.store.dto.part import BatchCreatePartRequest, CreatePartRequest
from hermetic_agent.store.services import ServiceContainer
...
def _extract_effective_mcp_token(request: Request) -> str | None:
    """Read request token, falling back to the container-level env var.

    顺序:
      1) request header (X-MCP-Token / Authorization / token);
      2) env 变量 ``MCP_TOKEN`` (容器级别).

    fallback env var 名固定为 ``MCP_TOKEN``. 业务 SKILL 的 env 注入由
    SKILL manifest 声明, 不在基座硬编码业务字段.
    """
    token = extract_mcp_token(request)
    if token:
        return token
    env_token = os.environ.get("MCP_TOKEN", "").strip() or None
    if env_token:
        logger.debug(
            "mcp_token_extracted",
            source="env:MCP_TOKEN",
            token_present=True,
...
async def chat_stream(request: Request) -> ResponseStream:
    ...
    async def streaming_fn(resp: ResponseStream) -> None:
        ...
        try:
            ...
            # 拦截链: Card → ask_user 转 card → tool_result 转 card → keepalive
            # L3 层拦截: tool_result → Card 渲染 (通过 CardRendererRegistry)
            # 必须在 ask_user intercept 之后, keepalive 之前
            intercepted = stream_with_ask_user_intercept(
                iterator, allowed_card_types=card_schemas,
            )
            card_rendered = stream_intercept_card_renderers(
                intercepted, session_id=session_id,
            )
            keepalived = stream_with_keepalive(card_rendered)
            try:
