...
from hermetic_agent.scenarios.injector import InMemoryAuditLogger, ScenarioInjector
...
def find_project_root() -> Path:
    ...
    Sanic 服务可能以多种方式启动 (``python -m hermetic_agent.main`` / 直接执行 main.py /
    IDE 调试器 / 容器),每次 cwd 都不一样。settings 里的 ``scenario_paths`` /
    ``work_root`` 都是相对路径,必须锚到项目根,否则 ``work/scenarios`` 会拼到
    ``src/hermetic_agent/work/scenarios`` (不存在)。
