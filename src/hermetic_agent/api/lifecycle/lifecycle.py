...
from hermetic_agent.audit.log.sys_logger import get_sys_logger
...
from hermetic_agent.store import SessionRepositoryFactory
...
async def startup(app: Sanic, settings: Any) -> None:
    ...
    from hermetic_agent.skills.runtime import FragmentLoader, PromptBuilder
    ...
    from hermetic_agent.auip import CARD_TYPES_SET
    ask_user_schema = {
        "type": "object",
        "required": ["card_type"],
        "properties": {
            "card_type": {
                ...
                "description": (
                    "Which kind of UI card to show the user. Frontend renders by "
                    "card_type. Built-in types: CHAT_FALLBACK / OD_INPUT / "
                    "QUESTION / TODO_LIST. Business types are registered by the "
                    "active SKILL at startup (see CardType self-registration)."
                ),
            },
            "title": {"type": "string"},
            ...
            "actions": {"type": "array"},
            "fields": {"type": "array", "description": "Form fields for OD_INPUT cards."},
    ...
    mcp_registry.register_synthetic_tool(
        ...
        description=(
            ...
            "For data presentation, pick a card_type registered by the active "
            "SKILL and put the structured data in `body`."
        ),
    ...
    from hermetic_agent.api.lifecycle.scenario_lifecycle import init_scenarios
    ...
    from hermetic_agent.audit.log.setup import setup_log_platform

    await setup_log_platform(settings)
    logger.info(LM.APP_READY)

    # Nacos 配置中心 + AI 注册表 (MCP/Agent/Skill/Prompt 同步)
    from hermetic_agent.config.nacos_client import NacosClient
    from hermetic_agent.core.nacos_ai_sync import NacosAISync
    ...
    if nacos_ok:
        ...
        # 发布当前配置到 Nacos 配置中心
        from pathlib import Path
        cfg_file = Path("work/nacos/hermetic_agent.yaml")
        if cfg_file.exists():
...
async def shutdown(app: Sanic) -> None:
    ...
    from hermetic_agent.audit.log.setup import shutdown_log_platform
...
def _init_turn_subsystem(app: Sanic, settings: Any) -> None:
    ...
    """
    from hermetic_agent.core.suspendable_scheduler import SuspendableScheduler
