...
from hermetic_agent.api.app.blueprint_registry import register_all_blueprints
from hermetic_agent.api.http.logging_setup import configure_logging
from hermetic_agent.api.lifecycle.lifecycle import shutdown, startup
from hermetic_agent.api.lifecycle.readiness import build_ready_response
...
def _configure_logging(settings: Settings) -> None:
    """薄壳: 委托给 ``hermetic_agent.api.http.logging_setup.configure_logging``。
...
def create_app(settings: Settings | None = None) -> Sanic:
    ...
    logger.debug("application_create_start", app_name="hermetic-agent")

    app = Sanic("hermetic-agent")
    ...
    # ``hermetic_agent.api.app.app.blueprint_registry.register_all_blueprints`` —
    ...
    from hermetic_agent.audit.log.log_middleware import LogMiddleware
    from hermetic_agent.audit.log.request_logger import init_request_logger

    init_request_logger(settings.log_system_type)
    from hermetic_agent.audit.log.request_logger import get_request_logger
    ...
    logger.debug("application_create_completed", app_name="hermetic-agent")
    return app
