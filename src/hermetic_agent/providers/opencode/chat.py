...
from hermetic_agent.providers.streaming import (
    OPENCODE_STREAM_END,
    StreamEvent,
    map_opencode_event,
)
...
# 缓存: 同一 opencode 节点 (按 base_url 标识) 上一次写入的 SKILL env.
# 同一个用户连发 N 条消息, 只在 env 变化时调一次 admin API 写 env + reload.
# 避免每条 chat 都触发 opencode 重启 (~1s 卡顿).
_SKILL_ENV_LAST_WRITTEN: dict[str, dict[str, str]] = {}

# Admin server 监听端口 (跟 docker/opencode/scripts/admin_server.py / docker-compose.yml 一致)
...
# 向后兼容: 旧代码 ``OPENCODE_ADMIN_PORT`` 直接引用模块名的也工作.
# 优先用上面的 _opencode_admin_port().
...
async def _push_skill_envs_to_opencode(
    agent_base_url: str,
    skill_envs: dict[str, str],
) -> bool:
    """把业务 SKILL 声明的环境变量写进 opencode 容器的 env.runtime + reload.

    业务 SKILL 在 ``skill.yaml`` 的 ``required_envs`` 段声明自己需要的 env,
    Hub 在每个 chat 之前把它写到 opencode 容器的 env.runtime, 然后触发
    reload. 触发条件: 同 base_url 上次的 env dict != 现在的 dict (任一
    字段新增/删除/值变化都触发).

    Args:
        agent_base_url: e.g. ``http://opencode-1:14096``. 我们推导 admin 端口.
        skill_envs: 业务 SKILL 声明的 env dict, 形如 ``{"API_KEY": "xxx",
            "API_BASE": "https://..."}``. 空 dict 跳过.

    Side effects:
        - HTTP POST 到容器内 :7778/admin/env (写 env.runtime)
        - HTTP POST 到容器内 :7778/admin/reload (SIGTERM + 1s 后自启)
        失败时仅 logger.warning, 不抛 — 当前 chat 仍然能跑 (业务 SKILL 的
        MCP 调外部 HTTP 时会用旧 env / 没 env, 由 SKILL.md 错误处理兜底).
    """
    if not skill_envs:
        return False

    last = _SKILL_ENV_LAST_WRITTEN.get(agent_base_url)
    if last == skill_envs:
        return False  # env 没变, 跳过
    ...
    try:
        ...
        logger.warning("push_skill_envs_url_parse_failed", error=str(e))
        return False

    headers = {"Content-Type": "application/json"}
    env_body = dict(skill_envs)
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r1.status_code >= 400:
                logger.warning(
                    "push_skill_envs_env_write_failed",
                ...
                return False
            logger.info(
                "push_skill_envs_env_written",
                agent_base_url=agent_base_url,
                admin_url=admin_url,
                env_keys=list(env_body.keys()),
            )
            ...
            if r2.status_code >= 400:
                ...
                return False
            logger.info(
                "push_skill_envs_reload_triggered",
                agent_base_url=agent_base_url,
                admin_url=admin_url,
            )
            s = _settings()
        ...
        logger.warning(
            "push_skill_envs_admin_unreachable",
            agent_base_url=agent_base_url,
            error=str(e),
        )
        return False
    ...
    if not reload_healthy:
        logger.warning(
            "push_skill_envs_reload_health_timeout",
            agent_base_url=agent_base_url,
        )
        return False

    _SKILL_ENV_LAST_WRITTEN[agent_base_url] = dict(skill_envs)
    logger.info(
        "push_skill_envs_ok",
        agent_base_url=agent_base_url,
        env_keys=list(env_body.keys()),
...
def _build_runtime_context(
    ...
    ctx = (
        "\n\n<runtime-context>\n"
        "AUTH: 鉴权由 hermetic_agent / opencode 运行时配置处理，凭证来自容器环境变量，"
        "不会也不应该出现在对话内容里。\n"
...
async def blocking_chat(
    ...
    *,
    ...
    # 业务 SKILL 声明的 env 推到 opencode 容器 env.runtime, 触发 reload.
    # 基座不解释任何业务, 仅做 "key-value 写入" + "reload 触发" 物理操作.
    # 业务含义 (env 名 / value 怎么用) 由 SKILL 自己负责.
    if mcp_token:
        # 兼容旧 API: mcp_token 作为通用 key-value 推送 (老代码叫 token, 实际是任意 env).
        env_changed = await _push_skill_envs_to_opencode(
            agent_base_url=session_info.agent_base_url,
            skill_envs={"MCP_TOKEN": mcp_token},
        )
        if env_changed:
        ...
        try:
            ...
            async with hub.subscription(
                ...
                async for event in sub_iter:
                    ...
                    #
                    # 注意: timeout 跟注释里说的"~1s"严格对齐. 不要随意拉到
                    # 3s/5s — 真实错误 (非 race) 会被错误吞掉, 前端 30-60s
                    # 后才看到 5xx, 体验更差.
                    if (
                        mapped.type == "error"
                    ...
                    #
                    # 业务 tool_result 的卡渲染由 L3 层
                    # ``stream_intercept_card_renderers()`` 接管,
                    # 不在 L4. 详见 docs/core-skill-boundary.md.
                    if mapped.type == "tool_use":
                        ...
                        if mapped.data.get("tool_name") == "bash":
                            ...
                            if isinstance(cmd, dict):
                                ...
                                last_bash_command = ""
                    # L3 层的 ``stream_intercept_card_renderers()`` 负责
                    # tool_result → Card 渲染. L4 只发 raw events.
                    yield mapped
        finally:
                ...
                try:
                    ...
                    for part in raw_result.get("parts", []):
                        ...
                        if mapped_part is None:
                            continue
                        # L3 层负责 Card 渲染. L4 只发 raw events.
                        yield mapped_part
                except Exception as task_err:
                    logger.warning(
                        "opencode_chat_task_failed",
