...
- **项目名称**：OpenCode Agent Scheduler Hub（fh-hermetic_agent）
...
1. **入口与运行**：`src/hermetic_agent/main.py` 调 `create_app` 启 Sanic；容器化（`docker/hermetic-agent/Dockerfile`）构建；`docker-compose.yml` 编排 Hub + opencode-sandbox +（可选）frontend。
...
### [src/hermetic_agent/api/routes.py:53 + src/hermetic_agent/api/app.py:163-174] 整份 routes.py 死代码
- **问题描述**：`src/hermetic_agent/api/routes.py`（**1095 行**）定义了一个 `router = Blueprint("agent_scheduler", url_prefix="/agent")`，挂在 `/agent/chat`、`/agent/chat/stream`、`/agent/session/*`、`/agent/skills`、`/agent/tools`、`/agent/pool/*` 等所有核心端点，但 `app.py:create_app` **完全没有 `app.blueprint(router)`**——只挂了 `chat_bp`/`session_bp`/`registry_bp`/`pool_bp` 等新拆分的 blueprint。`routes.py` 既未 import，也未注册。`/agent/chat` 实际由 `controllers/chat_controller.py` 提供。`routes.py` 的 `ChatRequest`/`ChatResponse` 跟 `schemas.py` 中同名，导入冲突。
- **影响场景**：
...
- **修复建议**：
  1. **删除** `src/hermetic_agent/api/routes.py` 整个文件（1095 行），并把 `_extract_mcp_token` 与 `_resolve_session_directory` 移到一个独立 `api/extractors.py` 供 `chat_controller.py` 引用；
...
### [src/hermetic_agent/skill_runtime/fragments.py:192-204] 死代码 — 复制粘贴残留
...
### [src/hermetic_agent/api/controllers/chat_controller.py:436-477] SSE keepalive 在错误路径下重复写 done，可能泄漏连接
...
### [src/hermetic_agent/api/controllers/chat_controller.py:176-189] 中文/英文路由正则可被构造绕过
...
### [src/hermetic_agent/api/controllers/auth_controller.py:296-300] 日志把 feihe 整个 response body 记录（含 token）
- **问题描述**：
  ```python
  logger.info(
...
- **修复建议**：
  ...
  2. 改用 `redact_value`：
  ```python
  from hermetic_agent.policy.audit import redact_value
...
### [src/hermetic_agent/api/controllers/auth_controller.py:158-162] MD5 哈希密码——算法过时
...
### [src/hermetic_agent/providers/opencode_chat.py:1040-1065] race 抑制逻辑可能误吞真实错误
- **问题描述**：
  ```python
  if (
      mapped.type == "error"
...
### [src/hermetic_agent/providers/opencode_chat.py:1133-1144] 不完整任务取消可能让 HTTP client 留 leak
- **问题描述**：
  ```python
  else:
      chat_task.cancel()
...
### [src/hermetic_agent/api/app.py:60-63] 兜底 SanicException 类型断言是潜在 silent failure
- **问题描述**：
  ```python
  try:
...
### [src/hermetic_agent/providers/launcher.py:139-150 + 217-229] Popen + 信号处理，孤儿进程风险
- **问题描述**：
  ```python
...
### [src/hermetic_agent/scenarios/middleware.py:43-49] 中间件热重载引用缓存可泄漏旧 router
- **问题描述**：
  ```python
...
### [src/hermetic_agent/api/controllers/chat_controller.py:342-358] 重复桥接的 agent_name 查找可能 N² 慢
...
### [src/hermetic_agent/api/routes.py:53 整体] 1095 行单文件，违反 L1 ≤ 200 行硬限
- **问题描述**：`scripts/ci_check.py` 明确把 L1 上限定为 200 行（`LINE_LIMITS["L1"] = 200`），但 `routes.py` 是 1095 行。当前**确实没注册**到 app，但 `LINE_LIMITS` 检查只对"实际挂载的"路径生效还是全部生效？根据 `LAYER_PATTERNS["L1"] = ["hermetic_agent/api/"]`，**所有 `api/` 下的文件都计**——`routes.py` 应该被 CI 拦下。
- **影响**：CI 可能没跑 / 有豁免；架构纪律性受损。
- **建议**：彻底删除（见 🔴 项 #1），或拆成 controllers。

### [src/hermetic_agent/api/controllers/chat_controller.py:836] 836 行单文件
...
### [src/hermetic_agent/providers/opencode_chat.py:1206] 1206 行单文件
...
### [src/hermetic_agent/api/app.py:87-105] 异常处理器把 traceback 返回给客户端
- **问题描述**：
  ```python
  return JSONResponse({
...
### [src/hermetic_agent/api/controllers/chat_controller.py:680-686] 用 `err.body.decode()` 反向解析 JSON
- **问题描述**：
  ```python
  await resp.write(
      StreamEvent.error(
...
### [src/hermetic_agent/api/lifecycle.py:91-115] 注册默认 agent 时 `bridge.register` 静默吞错
- **问题描述**：
  ```python
  try:
      bridge.register(cfg)
...
### [src/hermetic_agent/streaming.py:472] 模块级 dataclass StreamEvent 与 `_to_dict` 通用
...
### [src/hermetic_agent/api/controllers/registry_controller.py:82] 与 routes.py 重复（已合并到 🔴 #1）
...
### [src/hermetic_agent/scenarios/router.py:89-99] URL/header/body 路由短路后未保存 `routing_context.rejected`
...
### [src/hermetic_agent/api/controllers/chat_controller.py:180-182] 日期正则匹配 "12-13" 也算合法
...
### [src/hermetic_agent/sandbox/runtime.py:240-262] docker run 命令中 env 透传缺转义
...
### [src/hermetic_agent/policy/engine.py:51-101] merge 函数是"只收紧"逻辑但合并时机不明确
...
### [src/hermetic_agent/api/scenario_lifecycle.py:114] 初始化失败仅日志
- **问题描述**：
  ```python
...
### [src/hermetic_agent/api/controllers/question_controller.py:53-61] 404 逻辑被一段"if not items and not filtered"反向触发
- **问题描述**：
  ```python
...
### [src/hermetic_agent/core/session.py:24-29] Vendor stub 几乎全 `NotImplementedError`
- **问题描述**：`src/hermetic_agent/_vendor/opencode.py` 是个完整 stub，覆盖 `AsyncOpencode` + `_SessionStub` 所有方法——但生产用法 4 个文件（`core/session.py` / `providers/opencode_chat.py` / `providers/opencode_adapter.py` / `providers/opencode_native_sdk.py`）都有 `try: from opencode_ai; except: from hermetic_agent._vendor.opencode`。
- **影响**：`opencode-ai` 包必须装才能跑（依赖里有 `opencode-ai>=0.1.0a0`）——vendor stub 永远走不到，是死代码。
- **建议**：删 `src/hermetic_agent/_vendor/` 整个目录。

### [src/hermetic_agent/providers/opencode_chat.py:779-780] 函数体内 import
...
### [src/hermetic_agent/skill_runtime/fragments.py:31-32] 粗略 token 估算常数
- **问题描述**：
  ```python
...
### [src/hermetic_agent/api/controllers/chat_controller.py:275-325] `_turn_event_to_sse` 跟 `turn_routes.py:60-108` 完全相同
...
### [src/hermetic_agent/api/controllers/chat_controller.py:466-477] heartbeat 注释已过时
...
### [src/hermetic_agent/api/controllers/chat_controller.py:843-846] `content_type="text/event-stream"` 硬编码
...
### [src/hermetic_agent/store/postgres.py:18-48] SCHEMA 常量内嵌 IF NOT EXISTS
...
### [src/hermetic_agent/api/controllers/scenario_controller.py:182 + 309] `from hermetic_agent.scenarios.loader import _validate_resources  # type: ignore`
...
### [src/hermetic_agent/api/controllers/chat_controller.py:151-174] `_extract_effective_mcp_token` 在热路径调 `get_settings()` 多次
- **问题描述**：每次 chat 都 `from hermetic_agent.config.settings import get_settings` + `get_settings().flight_mcp_token_env`——`get_settings` 有 `lru_cache`，但**导入本身**会做首次的 `.env` 解析副作用。
- **建议**：cache `_TOKEN_ENV_NAME` 模块级常数；或在 `lifespan startup` 阶段锁定。

### [src/hermetic_agent/api/scenario_lifecycle.py:23-38] `find_project_root` 可能在 PyInstaller / VFS 打包下失效
...
- **建议**：在 `pyproject.toml` 加 `[tool.hatch.build.targets.wheel] packages = ["src/hermetic_agent"]` + `mypy src/hermetic_agent` 作为 pre-commit + CI gate。

## 🟢 可选优化（风格 / 性能 / 微调）

### [src/hermetic_agent/api/controllers/chat_controller.py:43] `import hermetic_agent.auip.cards` 的 CARD_TYPES_SET 是 frozen set
- **建议**：已经是 `frozenset`（`cards.py:51`），OK。可考虑 cache 排序结果 `sorted(CARD_TYPES_SET)` 避免每次重复。

### [src/hermetic_agent/api/controllers/chat_controller.py:189] `_should_bypass_hitl_placeholder` 是模块级函数但实际跟 controller 强耦合
- **建议**：移进 `chat_controller.py` 内的私有方法，或移到 `scenarios/heuristics.py`。

### [src/hermetic_agent/api/lifecycle.py:21-52] `_skill_paths_with_fallbacks` 用 set 记 seen 后转 list
- **建议**：用 `dict.fromkeys(...)` 单次 O(N) 去重保序。

### [src/hermetic_agent/skills/registry.py:18-43] DEFAULT_TEMPLATE 是死代码
- **建议**：删除；改用 YAML frontmatter 直接读 `SKILL.md`。

### [src/hermetic_agent/scenarios/loader.py:57-68] YAML resolver monkey-patch 影响全局
- **建议**：改用 `yaml.compose` 或在每个 `load` 临时 patch，避免污染全局状态。

### [src/hermetic_agent/api/turn_routes.py:166-244] resume_turn 内嵌的 `_err_stream` 写法绕
- **建议**：抽到 `api/turn_routes.py` 模块顶部作为普通函数。

### [src/hermetic_agent/providers/opencode_chat.py:122] `_is_transient_opencode_error` 字符串匹配脆弱
- **建议**：用 `isinstance(e, httpx.TransportError)` 等类型判断代替字符串子串匹配。

### [src/hermetic_agent/auip/flight_card.py:35-52] `_parse_minutes` 重复于 `flight_query_presenter._parse_duration`
- **建议**：抽到 `auip/_duration.py` 共享。

### [src/hermetic_agent/auip/flight_query_presenter.py:13-37] 大段 magic string / constant
- **建议**：拆成独立的 `flight_labels.py` + `flight_classifier.py` 让 unit test 容易。

### [src/hermetic_agent/api/logging_setup.py:57-90] `_compact_event_renderer` 拼 markup 字符串
- **建议**：当前实现可读但每次都遍历 event_dict；可考虑把 `event` 字段提取出来用 `rich.console.Console().render` 一次渲染。

### [src/hermetic_agent/api/scenario_lifecycle.py:71-89] scenarios 加载路径不递归
- **建议**：与 `registry.load_from_paths` 一致用 `rglob` 支持子目录。

### [src/hermetic_agent/api/controllers/chat_controller.py:735-737] `if "ask_user" not in scenario_tools` 检查 O(N)
- **建议**：`tools_set = set(scenario_tools)` 再 `add()`。

### [src/hermetic_agent/providers/opencode_chat.py:60] `_FLIGHT_CARD_EMITTED: set[str]` 全局可变状态
- **建议**：移进 `OpenCodeAdapter._sessions` 的元数据，避免跨 session 干扰。

### [src/hermetic_agent/scenarios/config.py:191-236] `_cross_field` 警告不抛
...
2. **settings 中心化**（`hermetic_agent/config/settings.py` 526 行 + `env_sources.py` path-aware）——Hub 任何配置改动唯一入口；pydantic-settings 兜底 + `.env` 自动加载。
3. **Sensitive 信息脱敏**（`hermetic_agent/policy/audit.py` `redact_value` + `redact_path` + `redact_obj` 三层递归）——路径/字段双维度，路径还按 env-file/pem/ssh-key/generic 分类，**这是 Hub 部署到企业内网时的合规底气**。
...
- `src/hermetic_agent/api/routes.py`: 🔴 **高**（1095 行死代码 + 重复路由 + 双份 Pydantic model）
- `src/hermetic_agent/api/controllers/chat_controller.py`: 🔴 **高**（836 行 + SSE 状态机混乱 + chat→agent O(N) + race 抑制边界 + 路由 regex 过宽）
- `src/hermetic_agent/providers/opencode_chat.py`: 🔴 **高**（1206 行 + Popen 阻塞 + 资源管理边界 + race 抑制 3s vs 注释 1s）
- `src/hermetic_agent/api/controllers/auth_controller.py`: 🔴 **高**（明文 token 落日志 + MD5 协议依赖 + 整段 raw response 落 body）
- `src/hermetic_agent/skill_runtime/fragments.py`: 🔴 **高**（死代码 12 行 + token 估算粗略）
- `src/hermetic_agent/providers/launcher.py`: 🟡 **中**（Popen 同步阻塞 + 路径校验不查 symlink）
- `src/hermetic_agent/scenarios/middleware.py`: 🟡 **中**（router 引用缓存可泄漏旧对象）
- `src/hermetic_agent/policy/`: 🟢 **低**（设计扎实，audit 脱敏细致）
- `src/hermetic_agent/scenarios/`: 🟢 **低**（配置 Pydantic 校验强，loader 边界清晰）
- `src/hermetic_agent/store/`: 🟢 **低**（抽象简洁，Postgres + Memory 双实现，asyncpg 池管理得当）
- `src/hermetic_agent/auip/`: 🟢 **低**（卡片协议清晰，presenter 集中业务规则）
- `src/hermetic_agent/api/lifecycle.py`: 🟡 **中**（`bridge.register` 静默吞错 + skill 加载路径 fallback 不去重）
- `src/hermetic_agent/api/scenario_lifecycle.py`: 🟡 **中**（初始化失败仅日志，行为不明确）
- `src/hermetic_agent/api/logging_setup.py`: 🟢 **低**（设计优雅，Rich 主题 + structlog 双模式）
- `src/hermetic_agent/core/`: 🟡 **中**（scheduler / turn_store / suspendable_scheduler 三个文件都偏小，in-memory 实现可接受；`session.py` 是 1.x 时代残留）
- `src/hermetic_agent/sandbox/runtime.py`: 🟡 **中**（env 拼 argv 缺转义 + `_node_health_port` 端口从 URL 解析不可靠）
...
1. **删除 `src/hermetic_agent/api/routes.py` 整段死代码**（1095 行 + 重复路由 + 重复 Pydantic model），把 `_extract_mcp_token` / `_resolve_session_directory` 提到独立 `api/extractors.py`。建议紧接着跑一次 `pytest -k routes` + `curl /agent/chat` 端到端验证删除后系统照常。
