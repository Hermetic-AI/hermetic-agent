...
description: IATA 3 字码 / ICAO 4 字码 ↔ 中文城市名翻译对照表。**v1.4.0 角色变更**:MCP `queryFlightBasic` 不再接受 IATA 码,本 skill 降级为「用户说 IATA/ICAO/机场名 → LLM 翻成中文再发 MCP」的翻译辅助。父 skill `flight-query` 调用本子 skill 做翻译。
version: 1.4.0
allowed-tools: []
---
...
> **加载时机**:父 skill `flight-query` 提示"详见 `flight-query:iata_icao_codes`"时;或 LLM 遇到未识别城市 / 用户说 IATA/ICAO 码要翻译成中文时。
> 
> **本文档不重复**:endpoint / 协议 / token 契约(见父 skill `flight-query` §1 §2)。
>
> **v1.4.0 变更**:之前 IATA 是直接发 MCP 的字段名,现在 MCP 改要中文城市名,本表是「IATA → 中文」的翻译辅助,**不**再是直接调用参数。
...
| 用途 | **客运系统标准**:机票/登机牌/MCP 接口/航司显示 | **航空运行标准**:飞行计划/ATC 雷达/航管通信 |
| 例子(北京首都) | `PEK` | `ZBAA` |
| **MCP `query_flight_basic` 接受** | ✅ **是**(本 skill 唯一依赖) | 🟡 未实测(可能不接受) |

> **铁律**:**优先 IATA 3 字码**。MCP 接口只验过 IATA。ICAO 仅在 IATA 不明 / 文档明确支持时再用。
